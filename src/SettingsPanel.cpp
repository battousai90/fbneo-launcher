...
SettingsPanel::SettingsPanel() : Box(Gtk::ORIENTATION_VERTICAL, 10) {
    ...
    // --- ROMs Directory ---
    grid->attach(m_label_roms, 0, 0, 1, 1);           // Label: Column 0
    m_label_roms.set_halign(Gtk::ALIGN_START);         // Align label to the left
    grid->attach(m_entry_roms, 1, 0, 1, 1);            // Entry: Column 1
    m_entry_roms.set_hexpand(true);                    // Expand entry to fill space
    grid->attach(m_button_browse_roms, 2, 0, 1, 1);    // Button: Column 2

    m_button_browse_roms.set_image_from_icon_name("folder-open-symbolic", Gtk::ICON_SIZE_BUTTON);
    m_button_browse_roms.set_always_show_image(true);
    m_button_browse_roms.set_label("Browse");
    m_button_browse_roms.signal_clicked().connect([this] {
        on_folder_clicked(&m_entry_roms);
    });

    // --- DAT File ---
    grid->attach(m_label_dat, 0, 1, 1, 1);
    m_label_dat.set_halign(Gtk::ALIGN_START);
    grid->attach(m_entry_dat, 1, 1, 1, 1);
    m_entry_dat.set_hexpand(true);
    grid->attach(m_button_browse_dat, 2, 1, 1, 1);

    m_button_browse_dat.set_image_from_icon_name("document-open-symbolic", Gtk::ICON_SIZE_BUTTON);
    m_button_browse_dat.set_always_show_image(true);
    m_button_browse_dat.set_label("Select");
    ...
    // --- Thumbnails ---
    grid->attach(m_label_thumbs, 0, 2, 1, 1);
    m_label_thumbs.set_halign(Gtk::ALIGN_START);
    grid->attach(m_entry_thumbs, 1, 2, 1, 1);
    m_entry_thumbs.set_hexpand(true);
    grid->attach(m_button_browse_thumbs, 2, 2, 1, 1);
